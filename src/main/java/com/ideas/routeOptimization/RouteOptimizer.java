...
public class RouteOptimizer {
	...
	public ArrayList<TreeMap<Time, ArrayList<ArrayList<DataPoint>>>> optimizeRoute()
		...
		System.out.println(timesMapOutput);
		...
		for (int i = 0; i < vList.size(); i++) {
			if (vList.get(i).size() == 2) {
				...
				try {
					...
					if ((minDistance + distanceCentroidIdeas) > 2 * distanceActualCentroidIdeas) {
		...
		for (int i = 0; i < vList.size(); i++) {
			if (vList.get(i).size() == 1) {
				...
				try {
					...
					if ((minDistance + distanceCentroidIdeas) > 2 * distanceActualLocIdeas) {
	...
	private static ArrayList<ArrayList<DataPoint>> basicClustering(
		...
		if (arrayList.size() > 1)
			numberOfClusters = Math.max(2, arrayList.size() / 3);
