	...
	public void createExcel() {
		try {
			...
			Connection con = DriverManager.getConnection(
					"jdbc:mysql://localhost:3306/cabservice", "root", "root");
			...
			for (int numSheets = 0; numSheets < 3; numSheets++) {
				...
				ResultSet rs = st
						.executeQuery("SELECT info.username, info.address, info.mobile,travel_date,EVENT,TIME FROM employee_info info INNER JOIN employee_dashboard dashboard ON (info.username = dashboard.username) "
				...
				String objCol2 = "Username" + "," + "Address" + "," + "Mobile"
						+ "," + time;
				...
				for (i = 3; i < dataSheet1.keySet().size() - 2; i++) {
					...
					if (!lastUserName.equalsIgnoreCase((String) objArr[0])) {
						...
						for (int column = 0; column < 3; column++) {
							...
							cell.setCellValue((String) objArr[column]);
						}
					...
					for (int column = 3; column < cellNum; column = column + 2) {
						...
						try {
							...
							if (firstRow.getCell(column).getStringCellValue()
									.equalsIgnoreCase((String) objArr[3])) {
								for (int subColumn = column; subColumn <= column + 1; subColumn++) {
									...
									if (secondRow
											...
											.equalsIgnoreCase(
													(String) objArr[4])) {
										cell.setCellValue((String) objArr[5]);
									}
								}
							}
